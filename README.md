...
- **專案儲存／開啟**：可把編輯狀態存成 `.json` 專案檔，下次繼續編輯；GUI 與網頁版格式通用，可互相開啟。網頁版另外會自動存到瀏覽器，關閉頁面再開會自動復原。
- **批次刪除句子**：全選／清除勾選／刪除勾選；網頁版每句另有 ✕ 單句刪除。
- **查字典（族語 E 樂園）**：編輯區每句可開內嵌字典，依目前語別查 Klokah 公開 API；可改關鍵字（中文或族語），點「套用」寫回該行翻譯。
